...
console.log("Attempting to connect to Analytics Service at:", ANALYTICS_URL);

if (!ANALYTICS_URL || ANALYTICS_URL.includes("127.0.0.1")) {
  console.warn(
    "AI Service URL is missing or pointing to localhost. Skipping AI step.",
  );
  // Return early or set aiInsights to a default message to avoid the crash
}
...
class AnalyticsController {
  async getStudentRiskAnalysis(req, res, next) {
    try {
      ...
      if (!doc.exists) {
        return res.status(200).json({
          ...
          data: {
            ...
            scoreAvg: 0,
            trendValue: 0,
            recommendation: "Complete an assessment to see analysis.",
          },
        });
      }
      ...
      const scores = Object.keys(resultsMap)
        ...
        .map((key) =>
          parseFloat(
            (resultsMap[key].score / resultsMap[key].answers.length) * 10,
          ),
        );
      ...
      const pythonResponse = await fetch(`${ANALYTICS_URL}/predict-risk`, {
        method: "POST",
      ...
      res
        .status(200)
        .json({ success: true, data: { ...prediction, history: scores } });
    } catch (error) {
  ...
  async getOverallStudentStats(req, res, next) {
    try {
      ...
      const pythonResponse = await fetch(
        `${ANALYTICS_URL}/overall-student-stats`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async getInstructorCourseReport(req, res, next) {
    try {
      ...
      console.log(
        `1. Found ${studentsSnapshot.size} grade documents in Firestore.`,
      );

      if (studentsSnapshot.empty) {
        return res
          .status(200)
          .json({ success: false, message: "No student records found." });
      }
      ...
      await Promise.all(
        studentsSnapshot.docs.map(async (doc) => {
          ...
          console.log(
            `2. Processing Student: ${studentId} | Attempts: ${resultsArray.length}`,
          );
          ...
          for (const result of resultsArray) {
            ...
            if (!assessmentTypeCache[aId]) {
              ...
              if (assessDoc.exists) {
                ...
                console.log(
                  `   - Assessment ${aId} detected as: "${assessmentTypeCache[aId]}"`,
                );
              } else {
                console.log(
                  `   - WARNING: Assessment ID ${aId} NOT FOUND in assessments collection.`,
                );
          ...
          if (filteredScores.length > 0) {
            ...
            const studentName = userDoc.exists
            ...
            studentsData.push({
              ...
              studentName,
              averageScore:
                filteredScores.reduce((a, b) => a + b, 0) /
                filteredScores.length,
              trend:
                filteredScores.length > 1
                  ? filteredScores[filteredScores.length - 1] -
                    filteredScores[0]
                  : 0,
            });
          }
      ...
      const pythonResponse = await fetch(
        `${ANALYTICS_URL}/instructor-course-report`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async getGroupedAssessmentReport(req, res) {
    try {
        ...
        return {
          ...
          avgScore:
            group.students.length > 0
              ? (total / group.students.length).toFixed(1)
              : 0,
      ...
      console.log(
        `2. Successfully grouped into ${finalData.length} assessment blocks.`,
      );
